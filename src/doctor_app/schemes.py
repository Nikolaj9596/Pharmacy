...
from src.doctor_app.types import STR_50
from src.types import POS_INT
...
class AppointmentCreateScheme(BaseModel):
    ...
    def validate_start_date(cls, value, values):
        if "end_date_appointment" in values and value >= values["end_date_appointment"]:
            raise ValueError("start_date_appointment must be less than end_date_appointment")
        if value <= datetime.now():
            raise ValueError("start_date_appointment must be greater than the current date")
        return value

    @validator("end_date_appointment")
    def validate_end_date(cls, value, values):
        if "start_date_appointment" in values and value <= values["start_date_appointment"]:
            raise ValueError("end_date_appointment must be greater than start_date_appointment")
       
        if value <= datetime.now():
            raise ValueError("end_date_appointment must be greater than the current date")
        return value
